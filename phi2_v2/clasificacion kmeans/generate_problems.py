...
class ProblemGenerator:
    def __init__(self):
        ...
        self.base_output_dir = "problemas_generados_phi"
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        
        print("="*70)
        print("GENERADOR CON PHI-1.5")
        print("="*70)
        ...
        for difficulty in self.difficulties:
            ...
            model_path = f'./models/phi-{difficulty}'
    ...
    def generate_problem(self, difficulty, num_problems=1):
        ...
        print(f"Generando {num_problems} problema(s) con Phi-1.5...")
        ...
        for i in range(num_problems):
            ...
            # Parámetros optimizados para Phi-1.5
            with torch.no_grad():
                output = model.generate(
                    ...
                    temperature=0.7,  # Phi funciona mejor con temp más baja
                    top_k=40,
                    top_p=0.9,
                    do_sample=True,
...
x2 >= 0;
    ...
    def interactive_generation(self):
        print("\n" + "="*70)
        print(" GENERADOR DE PROBLEMAS - PHI-1.5")
        print("="*70)
        
        available = list(self.model_paths.keys())
        if not available:
            print("\n No hay modelos Phi entrenados disponibles")
            print("   Ejecuta 'python fine_tune_phi.py' primero")
            return
        ...
        print(f"\n{'='*70}")
        print(f"Generando {cantidad} problema(s) con Phi-1.5...")
        print("="*70)
...
def main():
    try:
        ...
        if not generator.model_paths:
            print("\n No se encontraron modelos Phi-1.5 entrenados")
            print("   Ejecuta 'python fine_tune_phi.py' primero")
            return
        ...
        while True:
            ...
            continuar = input("\n¿Generar más problemas? (s/n): ").strip().lower()
